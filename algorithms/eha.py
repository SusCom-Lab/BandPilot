...
def _evaluate_global_bandwidth_eha(
    config: np.ndarray,
    ...
    当 global_mode=True 时，不仅评估当前配置的带宽，还评估采用当前配置后
...
def eha_search(
    ...
    if candidate_nodes:
        ...
        for node_id in candidate_nodes:
            selected = node_map[node_id][:gpu_need]
        ...
        if cluster_manager:
            ...
            for idx, config in enumerate(candidate_configs):
                if global_mode:
                    ...
                    bw = _evaluate_global_bandwidth_eha(
                        config, avail_gpu, num_dimensions, cluster_manager
                    )
                else:
                    # 普通模式：只评估当前配置
                    bw = cluster_manager.predict_with_contention(config)
                if bw > best_bw:
                    best_bw = bw
                    best_idx = idx
            ...
            part_bws, node_counts, total_counts = prepare_model_inputs(
                candidate_configs, total_gpu, gpu_bw_dict_list, switch_config, data_path
            )
    ...
    for node_group in itertools.combinations(sorted_nodes, k):
        ...
        # 核心思想：尝试均匀分配，如果失败则尝试±1/±2的不均匀分配变体
        ...
        # 2.2.1：添加基础均匀分配的所有排列
        for alloc in set(itertools.permutations(base_alloc)):
            allocation_variants.append(alloc)
        
        # 2.2.2：生成±1的不均匀分配变体
        # 对每个节点对(i,j)，尝试节点i+1，节点j-1，保持总和不变
        for i in range(k):
            for j in range(k):
                if i != j:  # 确保是不同的节点
                    ...
                    variant[i] += 1
                    variant[j] -= 1
                    # 检查总和是否正确，且每个节点的分配数 >= 0
                    if sum(variant) == gpu_need and all(x >= 0 for x in variant):
                        # 生成所有排列并去重
                        for perm in set(itertools.permutations(variant)):
                            if perm not in allocation_variants:
                                allocation_variants.append(perm)
        
        # 2.2.3：生成±2的不均匀分配变体
        # 对每个节点对(i,j)，尝试节点i+2，节点j-2，保持总和不变
        for i in range(k):
            for j in range(k):
                if i != j:  # 确保是不同的节点
                    variant = list(base_alloc)
        ...
        for alloc_variant in allocation_variants:
            ...
            if is_possible:
                ...
                config = np.zeros(num_dimensions, dtype=int)
                for idx in range(k):
                    _, gpu_list = node_group[idx]
                    num_to_take = alloc_variant[idx]
                    selected = gpu_list[:num_to_take]
                    config[selected] = 1
